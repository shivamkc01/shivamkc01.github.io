...
I decided to use a combination of [about pages](https://quarto.org/docs/websites/website-about.html) and [listing pages](https://quarto.org/docs/websites/website-listings.html) to replicate the site structure I enjoyed in my Apéro site. I used an about page layout for both my home and about pages, and listing pages to generate a list of blog posts, talks, publications, and projects. There was also some custom CSS involved, but we haven't gotten to that part of the blog post yet wink 😉 [Dear reader, it's at this point in my blog post drafting that I start realizing that what was meant to be a brief post is turning into a novel (*sigh*)]{.aside}

One of the aspects I liked about my Apéro site was that the About page highlighted the most recent posts from the blog, talk, publication, and project groups. In my Quarto site, I achieved this by including a one-entry listing grid for each group on my [About page](/about/index.qmd).
...
I may have published my brand new Quarto site in the fall of 2023, but I had actually taken my first go at porting it from Apéro in the fall of 2022. In an attempt to replicate the styling of my old site I tried to strong-arm my Quarto site into using tachyons^[A CSS design system used in the Hugo Apéro theme: <https://tachyons.io>], and I wrote **way too much** custom CSS in the process 😩.
...
// Colors
$spc-primary-light: #DBA979;
$spc-secondary: #0d6efd;
...
1. Run a keyword search for "figure-caption" in the [Bootstrap variables file](https://github.com/quarto-dev/quarto-cli/blob/main/src/resources/formats/html/bootstrap/_bootstrap-variables.scss). This will yield no results 🤔
1. Run the same keyword search in the [Bootstrap rules file](https://github.com/quarto-dev/quarto-cli/blob/main/src/resources/formats/html/bootstrap/_bootstrap-rules.scss) -- success! We have a clue, which is that `.figure-caption` is styled by a variable named `body-secondary` 🔍
...
1. Define your own `body-secondary` color variable in your custom theme file
    ...
    ```
1. Enjoy the fruits of your labor as a CSS detective 🕵🏽‍♀️
...
[Don't miss the [code annotations](https://quarto.org/docs/authoring/code-annotation.html) for this code chunk! 👀 <br>Such a cool feature!]{.aside}
